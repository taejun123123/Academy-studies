...
public class QnaDAO extends DAO{
	...
	// 1-2. 전체 데이터 개수 처리s0s0s
	// QnaController - (Execute) - QnaListService - [Qna.getTotalRow()]
	...
	// 2-1. 글보기 조회수 1증가 처리
	// QnaController - (Execute) - BoardViewService - [QnaDAO.increase()]
	...
	public QnaVO view(Long no) throws Exception{
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("예외 발생 :질문답변 보기 DB 처리 중 오류 발생");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt, rs);
		} // end of try ~ catch ~ finally
	...
	// 5. 글수정 처리
	// QnaController - (Execute) - QnaViewService - [QnaDAO.update()]
	...
	final String LIST = "" 
		+ " select no, title, id,name, writeDate, levNo "
		+ " from ( "
			+ " select rownum rnum, no, title, id,name, writeDate, levNo "
			+ " from ( "
				+ " select q.no, q.title, q.id,m.name, "
	...
	//질문답변 보기 + 수정 + 답변하기 할 때는 refNo ordNo levNo를 갖고와야됨 
