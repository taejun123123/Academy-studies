...
public class ImageDAO extends DAO{
	...
	public Long getTotalRow(PageObject pageObject) throws Exception{
		...
		try {
			...
			// 2. 연결
			con = DB.getConnection();
			System.out.println("imageDAO.getTotalRow().seq="+TOTALROW + getSearch(pageObject,true));
			// 3. sql - 아래 LIST
			// 4. 실행 객체 & 데이터 세팅
			pstmt = con.prepareStatement
					(TOTALROW + getSearch(pageObject,true)); // 전체 데이터 개수 퀴리인 경우 검색 조건이 있으면 where를 붙여라:true
			int idx = 0;
			idx = setSearchData(pageObject, pstmt, idx);
			// 5. 실행
			rs = pstmt.executeQuery();
	...
	public int write(ImageVO vo) throws Exception{
		...
		try {
			...
			e.printStackTrace();
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			throw new Exception("예외 발생 : 이미지 글등록 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	public int update(ImageVO vo) throws Exception{
		...
		try {
			...
			// 6. 표시 또는 담기
			if(result == 0) { // 글번호가 존재하지 않는다. -> 예외로 처리한다.
				throw new Exception("예외 발생 : 번호나 않거나 본인 글이 아닙니다. 정보를 확인해 주세요.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
	...
	public int delete(ImageVO vo) throws Exception{
		...
		try {
			...
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 이미지 게시판 삭제 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	// 6. 이미지 변경 처리
	// ImageController - (Execute) - ImageViewService - [ImageDAO.changeImage()]
	public int changeImage(ImageVO vo) throws Exception{
		...
		try {
			...
			result = pstmt.executeUpdate();
			// 6. 표시 또는 담기
			if(result == 0) { // 이미지 번호가 존재하지 않는다. -> 예외로 처리한다.
				throw new Exception("예외 발생 : 이미지가 없거나 번호가 맞지 않습니다. 정보를 확인해 주세요.");
			}
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 이미지 게시판 바꾸기 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	final String LIST = "" 
			...
			+ " from ( "
				...
				+ " from ( "
					+ " select i.no, i.title, i.id, m.name, to_char(i.writeDate ,'yyyy-mm-dd')writeDate, i.fileName "
	...
	private String getListSQL(PageObject pageObject) {
		...
		// 검색이 있는 경우 - where를 추가 안한다. :false
		sql += getSearch(pageObject,false);
		sql += " and (m.id = i.id)"; //조인을 하면 where 문이 반드시 나와야함
