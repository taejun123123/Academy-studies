...
public class MemberDAO extends DAO{
	...
	// 4. 회원정보 수정 처리
	// NoticeController - (Execute) - NoticeViewService - [NoticeDAO.update()]
	public int gradeNo(MemberVO vo) throws Exception{
		// 결과를 저장할 수 있는 변수 선언.
		int result = 0;
	...
	public Long getNewMsgCnt(String id) throws Exception{
		...
		try {
			...
			if(rs != null && rs.next()) {
				// rs -> vo
				newMsgCnt = rs.getLong(1);	 //데이터 하나만 끄낼때는 저렇게함.
			} // end of if
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
	...
	final String LIST =""
			+ " select id, name , birth, gender , tel,email ,gradeNo , gradeName, status, photo,email "
			+ " from ( "
			+ " select rownum runm,id, name , birth, gender , tel,email ,gradeNo , gradeName, status, photo "
			+ " from ( "
			+ " select m.id, m.name, "
			+ " to_char(m.birth, 'yyyy-mm-dd') birth,m.gender, m.tel,email, "
