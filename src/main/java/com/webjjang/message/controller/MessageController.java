...
public class MessageController {
	...
	public String execute(HttpServletRequest request) {
		...
		//로그인이 되어 있어야 메시지 메뉴가 보이고 처리가 가능하다. 그래서 if문으로 안물어봄
		...
		try { // 정상 처리
			...
			case "/message/view.do":
				...
				//JSP 보여지는 새로운 메시지를 다시 불러와서 세션을 넣는다.
				login.setNewMsgCnt((Long) Execute.execute(Init.get("/ajax/getNewMsgCnt.do"), id));
				
				//페이지 정보 - 리스트로 돌아갈 때 필요함.
				request.setAttribute("pageobj", PageObject.getInstance(request));
				jsp ="message/view";
				break;
				//writeForm은 리스트의 모달 창으로 대신 작성한다.
