...
public class MessageDAO extends DAO{
	...
	public int setReaded(Long no) throws Exception{
		// 결과를 저장할 수 있는 변수 선언. -1: 수정 성공 ->새로운 메세지개수 -1 처리 , 0: 수정 안됨
		int result = 0;
		...
		try {
			...
			else throw new Exception("예외 발생 : 메세지 읽기 표시 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	public MessageVO view(Long no) throws Exception{
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("예외 발생 :메세지 보기 DB 처리 중 오류 발생");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt, rs);
		} // end of try ~ catch ~ finally
	...
	public int decreaseNewMsgCnt(String id) throws Exception{
		...
		try {
			...
			// 5. 실행 - update : executeUpdate() -> int 결과가 나옴.
			result = pstmt.executeUpdate();
			System.out.println("MessageDAO.inceaseNewMsgCnt()-새로운 메시지 -1감소");
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 새로운 메시지 1감소 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
		...
		// 결과 데이터를 리턴해 준다.
		return result;
	} // end of dicreaseNewMsgCnt()

	
	
	
	// 3. 메시지 등록 처리
	// MessageController - (Execute) - MessageViewService - [MessageDAO.write()]
	public int write(MessageVO vo) throws Exception{
		...
		try {
			...
			e.printStackTrace();
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			throw new Exception("예외 발생 : 메시지 보내 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	// 3-1. 새로운 메시지가 전달된 회원 테이블의 새로운 메시지 갯수를 1증가 시킨다.
	// MessageController - (Execute) - MessageViewService - [MessageDAO.increase()]
	public int increaseNewMsgCnt(String id) throws Exception{
		...
		try {
			...
			// 5. 실행 - update : executeUpdate() -> int 결과가 나옴.
			result = pstmt.executeUpdate();
			System.out.println("MessageDAO.inceaseNewMsgCnt()-새로운 메시지 카운트 1증가");
		} catch (Exception e) {
			e.printStackTrace();
			// 특별한 예외는 그냥 전달한다.
			if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
			// 그외 처리 중 나타나는 오류에 대해서 사용자가 볼수 있는 예외로 만들어 전달한다.
			else throw new Exception("예외 발생 : 새로운 메시지 1증가 DB 처리 중 예외가 발생했습니다.");
		} finally {
			// 7. 닫기
			DB.close(con, pstmt);
		}
	...
	final String LIST = "" 
		+ " select  no, senderId, senderName,sendPhoto, sendDate, accepterId,accepterName,acceptPhoto,acceptDate "
		+ " from ( "
			+ " select rownum rnum, no, senderId, senderName,sendPhoto, sendDate, accepterId,accepterName,acceptPhoto,acceptDate "
			+ " from ( "
				+ " select m.no, m.senderId, sm.name senderName,sm.photo sendPhoto, "
				+ " to_char(m.sendDate, 'yyyy-mm-dd')sendDate, "
				+ " m.accepterId,am.name accepterName,am.photo acceptPhoto, "
	...
	private String getListSQL(PageObject pageObject) {
		...
		//mode 조건 처리 1.받은 메시지2.보낸 메시지 3.모든 메시지
	...
	final String INCREASENEWMSGCNT = "update member set newMsgCnt = newMsgCnt + 1 "
			+ " where id = ?"; 
	final String READED = " update message set acceptDate =sysdate "
			+ " where (no = ?) and acceptDate is null"; 
	final String VIEW= ""
			+ "select m.no,m.content, m.senderId, sm.name senderName,sm.photo sendPhoto, "
			+ " to_char(m.sendDate, 'yyyy-mm-dd')sendDate, "
			+ " m.accepterId,am.name accepterName,am.photo acceptPhoto, "
			+ " to_char(m.acceptDate, 'yyyy-mm-dd')acceptDate "
			+ " from message m,member sm,member am "
	...
	final String DELETE= "delete from board "
			+ " where no = ? and pw = ?"; 
	final String DECREASENEWMSGCNT = "update member set newMsgCnt = newMsgCnt - 1 "
			+ " where id = ?"; 
