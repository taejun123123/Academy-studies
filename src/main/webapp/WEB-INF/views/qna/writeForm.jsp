	...
	<div class="container">
		...
		<form action="write.do"method="post">
			...
			<div class="form-group">
				...
				<input class="form-control" name="title" id="title" required value="${(empty vo)?'':('[답변]' +=vo.title) }">
			</div>
			<div class="form-group">
				<label for="content">내용</label>
				<c:if test="${empty vo }">
					<textarea rows="3" class="form-control" name="content" id="content" required></textarea> 
				</c:if>			
				<c:if test="${!empty vo }">
					<textarea rows="3" class="form-control" name="content" id="content" required>
...
${vo.content }			
			...
			<button type="button" class="cancelBtn btn btn-info">취소</button>
		</form>
	</div>
</body>
</html>