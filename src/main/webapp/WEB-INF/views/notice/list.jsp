...
// 		$("#${pageObject.period}").prop('checked', true);
	...
	$(".noticeOption").change(function(){
		...
		if(this.optionList[1].checked){
			//alert("이전공지 보여주세요");
			location = "/notice/list.do?period=old";
		}
		if(this.optionList[2].checked){
			//alert("예정공지 보여주세요");
			location = "/notice/list.do?period=res";
		}
		if(this.optionList[3].checked){
			//alert("모든공지 보여주세요");
...
<!-- 라이어 버튼(현재공지,이전공지,예정공지,모든공지)  -->
	<c:if test="${!empty login && login.gradeNo == 9}">
	<form class="noticeOption">
		  <div class="custom-control custom-radio custom-control-inline">
		    <input type="radio" class="custom-control-input"  id="pre" name="optionList" value="현재공지">
		    <label class="custom-control-label" for="pre">현재공지</label>
		  </div>
		  <div class="custom-control custom-radio custom-control-inline">
		    <input type="radio" class="custom-control-input"  id="old" name="optionList" value="이전공지">
		    <label class="custom-control-label" for="old">이전공지</label>
		  </div>
		  <div class="custom-control custom-radio custom-control-inline">
		    <input type="radio" class="custom-control-input" id="res" name="optionList" value="예정공지">
		    <label class="custom-control-label" for="res">예정공지</label>
		  </div>
		  <div class="custom-control custom-radio custom-control-inline">
		    <input type="radio" class="custom-control-input"  id="all" name="optionList" value="모든공지">
		    <label class="custom-control-label" for="all">모든공지</label>
		  </div>
		</form>
		</c:if>
</c:if>
		<!-- 라이도 버튼(현재공지,이전공지,예정공지,모든공지) End  -->		
